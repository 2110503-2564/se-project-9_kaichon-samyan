    ...
    <main className="flex flex-col font-['Playfair_Display'] items-center px-4 sm:px-8 w-full max-w-3xl mx-auto pt-20">
      ...
      <div className="flex flex-col sm:flex-row items-center sm:items-start border border-gray-300 shadow-lg rounded-xl p-6 gap-8 bg-white w-full max-w-full">
        ...
        <div className="flex flex-col w-full sm:w-[300px] text-gray-800">
          <h1 className="font-medium text-4xl text-gray-900 mx-auto mb-4">
            {hotel.hotelName}
          </h1>
          <hr className="border-t-3 border-gray-800 mb-4 w-[100px] mx-auto" />
          <h2 className="text-lg font-medium text-gray-600 mb-2">
            Location:{" "}
            <span className="font-semibold text-gray-800">{hotel.address}</span>
          </h2>
          <h2 className="text-lg font-medium text-gray-600 mb-2">
            Website:{" "}
            <a
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center">
            <Link
              href={{ pathname: "/booking", query: { hotelId: hotel._id } }}
            >
              <button className="px-6 py-3 bg-black text-white rounded-xl shadow-lg hover:bg-gray-900 transition duration-300">
                Select Date
              </button>
            </Link>
            <div className="mt-4 sm:mt-0 text-xl font-semibold text-yellow-600">
              {getAverageRating()}
            </div>
          </div>
        </div>
        ...
        {ratings.map((rating, idx) => (
          <div
            ...
            className={`border p-6 rounded-lg shadow-md ${
              rating.user.name === currentUsername ? "bg-blue-50" : "bg-white"
            }`}
          >
            <div className="flex justify-between items-center">
              <div className="flex">
                <img
                  src={rating.user.profileImg || "/img/profilepic.jpg"}
                  alt="profilepic"
                  className="w-[30px] h-[30px] object-cover mr-2 rounded-[50%]"
                />
